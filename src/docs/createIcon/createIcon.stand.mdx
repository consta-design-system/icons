...
Свойство `name` отвечает за название иконки и добавляет его в `className`.
...
За цветовое решение для иконки отвечает параметр `color`. Варианты: `'mono'` или `'multiple'`.
...
Задекларируйте типы для иконок.
...
Используйте иконку как обычный компонент.
