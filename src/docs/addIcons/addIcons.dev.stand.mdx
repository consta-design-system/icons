...
2. Используйте функцию `createIcon`: передайте в неё пути к файлам с иконками в формате SVG. Это важный шаг, так как компоненты Consta принимают иконки определённого типа.
